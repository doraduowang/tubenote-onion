...
const DEFAULT_FOLDERS = ["All","Favorites"];
...
export default function App() {
  ...
  const [folders,setFolders] = useState(["All","Architecture","Philosophy","Favorites"]);
  ...
  const addFolder = () => {
    ...
    if(!f||folders.includes(f)){setFolderInput("");return;}
    setFolders(p=>[...p,f]);
    setFolderInput("");
  };
  const removeFolder = f => {
    if(f==="All"||f==="Favorites") return;
                  ...
                  <div className="nfolders">
                    {folders.map(f=>(
                      <div key={f} style={{display:"flex",alignItems:"center",flexShrink:0}}>
                        ...
                        {f!=="All"&&f!=="Favorites"&&(
                          <button onClick={()=>removeFolder(f)} style={{background:"none",border:"none",cursor:"pointer",color:"var(--ink-3)",fontSize:13,lineHeight:1,padding:"0 4px 0 0",marginLeft:-4,transition:"color var(--t)"}}
                    ...
                    <div style={{display:"flex",alignItems:"center",padding:"0 8px",flexShrink:0}}>
                      <input value={folderInput} onChange={e=>setFolderInput(e.target.value)}
                        onKeyDown={e=>{if(e.key==="Enter")addFolder();if(e.key==="Escape")setFolderInput("");}}
                        placeholder="+ folder" style={{width:72,padding:"3px 7px",border:"1.5px solid var(--border-s)",borderRight:"none",borderRadius:"4px 0 0 4px",background:"var(--surface)",color:"var(--ink)",fontFamily:"'IBM Plex Sans',sans-serif",fontSize:11,outline:"none"}}/>
                      <button onClick={addFolder} style={{padding:"3px 8px",border:"1.5px solid var(--border-s)",borderLeft:"none",borderRadius:"0 4px 4px 0",background:"var(--surface)",color:"var(--ink-3)",fontFamily:"'IBM Plex Sans',sans-serif",fontSize:11,cursor:"pointer",transition:"all var(--t)"}}
                        onMouseEnter={e=>{e.currentTarget.style.background="var(--lime)";e.currentTarget.style.color="#fff";e.currentTarget.style.borderColor="var(--lime)";}}
                        onMouseLeave={e=>{e.currentTarget.style.background="var(--surface)";e.currentTarget.style.color="var(--ink-3)";e.currentTarget.style.borderColor="var(--border-s)";}}>Add</button>
                    </div>
                  </div>
